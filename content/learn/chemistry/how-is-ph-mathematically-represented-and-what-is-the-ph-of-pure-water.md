---
_schema: default
id: 167120
date: 2025-01-31
title: How is pH mathematically represented and what is the pH of pure water?
article_title: How is pH mathematically represented and what is the pH of pure water?
date_format: MMM DD, YYYY
post_on_text: Posted on
seo:
  title: QuickTakes | Chemistry
  page_description: >-
    This content explains the mathematical representation of pH, how to calculate it using hydrogen ion concentration, and describes the pH of pure water as being neutral at 7.
  page_keywords: >-
    pH, hydrogen ion concentration, logarithm, pure water, neutral, acidity, basicity, pH scale, mathematical representation
author: QuickTakes
question: >-
    How is pH mathematically represented and what is the pH of pure water?
answer: >-
    The pH of a solution is mathematically represented as the negative logarithm (base 10) of the hydrogen ion concentration. The formula for calculating pH is given by:\n\n$$\n\text{pH} = -\log_{10}[\text{H}^+]\n$$\n\nIn the case of pure water, at 25°C, the concentration of hydrogen ions ($H^+$) is approximately $1 \times 10^{-7}$ molar. Using the pH formula, we can calculate the pH of pure water as follows:\n\n$$\n\text{pH} = -\log_{10}(1 \times 10^{-7}) = 7\n$$\n\nThus, the pH of pure water is 7, which is considered neutral on the pH scale. This scale ranges from 0 to 14, where values below 7 indicate acidity and values above 7 indicate basicity.
subject: Chemistry
file_name: how-is-ph-mathematically-represented-and-what-is-the-ph-of-pure-water.md
url: /learn/chemistry/questions/how-is-ph-mathematically-represented-and-what-is-the-ph-of-pure-water
score: -1.0
related_article1:
    id: 167116
    title: What role does hydrogen bonding play in the cohesion of water?
    subject: Chemistry
    url: /learn/chemistry/questions/what-role-does-hydrogen-bonding-play-in-the-cohesion-of-water
related_article2:
    id: 167116
    title: What role does hydrogen bonding play in the cohesion of water?
    subject: Chemistry
    url: /learn/chemistry/questions/what-role-does-hydrogen-bonding-play-in-the-cohesion-of-water
related_article3:
    id: 167125
    title: Explain how hyperventilation can lead to alkalosis and a shift in pH balance.
    subject: Chemistry
    url: /learn/chemistry/questions/explain-how-hyperventilation-can-lead-to-alkalosis-and-a-shift-in-ph-balance
related_article4:
    id: 167116
    title: What role does hydrogen bonding play in the cohesion of water?
    subject: Chemistry
    url: /learn/chemistry/questions/what-role-does-hydrogen-bonding-play-in-the-cohesion-of-water
related_article5:
    id: 167124
    title: What is the mechanism of hyperventilation and how does it affect blood chemistry?
    subject: Chemistry
    url: /learn/chemistry/questions/what-is-the-mechanism-of-hyperventilation-and-how-does-it-affect-blood-chemistry
---

&nbsp;
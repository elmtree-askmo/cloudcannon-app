    ...
    <div className={styles["carousel-container"]}>
      <div className={styles["carousel-center-container"]}>
        ...
        <div className={styles["carousel-box"]}>
          {isClient && (
            <Carousel dots={false} ref={carouselRef} autoplaySpeed={5000} waitForAnimate autoplay lazyLoad="progressive">
              {block.slides.map((item, index) => (
                <div className={styles["carousel-custom-item"]} key={index}>
                  <div className={styles["carousel-item-avatar"]}>
                    <Image src={item.avatar} alt={item.name[language] || item.name["en"]} width={100} height={100} loading={index === 0 ? "eager" : "lazy"} priority={index === 0} quality={75} />
                  </div>

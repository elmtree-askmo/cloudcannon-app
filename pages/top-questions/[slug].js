...
export default function TopQuestion({page, language}){
    useEffect(() => {
            ...
            <div className={styles['blog-content-container']}>
                <div className={styles['blog-content-center-container']}>
                    <h2 className={styles["blog-content-title"]}>
                        <Link href="/top-questions" className={styles["blog-back-btn"]} ><img src="/backIcon.svg" /></Link>
...
export async function getStaticPaths() {
	const slugs = (await filer.listItemSlugs('top-questions')).map((slug) => ({ params: { slug } }));
...
export async function getStaticProps({ params }) {
	const page = await filer.getItem(`${params.slug}.md`, { folder: 'top-questions' });
